...
// rawValue = KTC dynasty value (integer, same units as FantasyCalc API).

import type { RichRosterPlayer } from './reportCard';

export interface DTVSnapshot {
    preDraftTotal:  number;   // KTC dynasty value sum of pre-draft roster
...
export function computeDTVSnapshot(rosterRich: RichRosterPlayer[]): DTVSnapshot {
    ...
    const sign           = delta >= 0 ? '+' : '';
    const deltaLabel     = `${sign}${formatKTC(delta)} Dynasty Value Added`;
...
export function computePickDTVNote(rawValue: number, tier: number): string {
    ...
    const formatted = formatKTC(rawValue);
    if (tier === 1) return `High-value asset — ${formatted} KTC`;
    if (tier === 2) return `Strong dynasty piece — ${formatted} KTC`;
    if (tier === 3) return `Developmental asset — ${formatted} KTC`;
    return `${formatted} KTC trade value`;
}
