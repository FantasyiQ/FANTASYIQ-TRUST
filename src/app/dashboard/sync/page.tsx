...
export default function SyncPage() {
    ...
    async function handleLookup(e: React.FormEvent) {
        ...
        try {
            ...
            if (!res.ok) {
                setError(data.error ?? 'Failed to look up username');
                return;
            }
            setResult(data);
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-gray-500 text-sm">@{result.user.username} · {result.season} season · {result.leagues.length} league{result.leagues.length !== 1 ? 's' : ''}</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                            <div className="px-5 py-4 border-b border-gray-800 flex items-center justify-between">
                                ...
                                <button
                                    type="button"
                                    onClick={() => {
                                        if (selected.size === result.leagues.length) {
                                            setSelected(new Set());
                                        } else {
                                            setSelected(new Set(result.leagues.map((l) => l.league_id)));
                                        }
                                    }}
                            ...
                            {result.leagues.length === 0 ? (
                                <div className="px-5 py-10 text-center text-gray-500 text-sm">
                                    No leagues found for the {result.season} season.
                                </div>
                            ) : (
                                    ...
                                    {result.leagues.map((league) => (
                                        ...
                                        >
                                            ...
                                            {league.avatar ? (
                                                ...
                                                <div className="w-9 h-9 rounded-lg bg-gray-800 shrink-0 flex items-center justify-center text-gray-600 text-xs font-bold">
                                                    FF
                                                </div>
                                            )}
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-gray-500 text-xs mt-0.5">
                                                    {league.total_rosters} teams · {scoringLabel(league)}
                                                </p>
                                            </div>
                ...
                {step === 'done' && (
                    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-10 text-center space-y-4">
                        ...
                        <h2 className="text-xl font-bold">
                            {synced} league{synced !== 1 ? 's' : ''} synced!
                        </h2>
                        <p className="text-gray-400 text-sm">Your Sleeper leagues are now visible on your dashboard.</p>
                        <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
                            <Link
                                href="/dashboard"
                                className="bg-[#C8A951] hover:bg-[#b8992f] text-gray-950 font-bold px-6 py-2.5 rounded-lg transition text-sm"
                            >
                                Go to Dashboard
                            </Link>
                            <button
