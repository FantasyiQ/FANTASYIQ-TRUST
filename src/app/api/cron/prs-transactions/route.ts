...
export async function GET(request: Request): Promise<Response> {
    ...
    try {
        ...
        // One Sleeper API call per distinct league (multiple FiQ users can share a league)
        ...
        for (const { leagueId } of leagues) {
            try {
                const [rosters, transactions] = await Promise.all([
                    ...
                    getLeagueTransactions(leagueId, nflState.week),
                ]);

                // roster_id → sleeper owner_id (needed for fallback when consenter_ids absent)
                ...
                for (const txn of transactions) {
                    ...
                    if (txn.type === 'trade') {
                        // consenter_ids are sleeper user_ids who agreed to the trade.
                        // Fall back to resolving roster_ids → owner_ids if absent.
                ...
                if (rows.length > 0) {
                    ...
                    tradesWritten  += result.count > 0
                        ? rows.filter(r => r.eventType === 'trade_response').length
                        : 0;
                    waiversWritten += result.count > 0
                        ? rows.filter(r => r.eventType === 'waiver_active').length
                        : 0;
                }
            } catch (err) {
                captureError(err, { cron: 'prs-transactions', leagueId });
                leaguesFailed++;
            }
        }
