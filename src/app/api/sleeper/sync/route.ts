...
// POST /api/sleeper/sync — upsert selected leagues for the authenticated user
export async function POST(request: NextRequest): Promise<Response> {
    ...
    try {
        ...
        // Re-fetch to verify ownership and get full data
        ...
        if (toSync.length === 0) {
            return Response.json({ error: 'No valid leagues to sync' }, { status: 400 });
        }
...
// DELETE /api/sleeper/sync?leagueId=xxx — remove a synced league
